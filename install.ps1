...
try {
    ...
    if (Test-Path $scriptPath) {
        . $scriptPath
        Write-Host "[OK] Toolkit functions loaded successfully" -ForegroundColor Green
    } else {
        throw "ForensicFunctions.ps1 not found at $scriptPath"
    }
} catch {
    Write-Error "Failed to load toolkit functions: $($_.Exception.Message)"
    exit 1
}
...
if (-not $SkipTools) {
    ...
    try {
        ...
        Write-Host "[OK] Forensic tools installation completed" -ForegroundColor Green
    } catch {
...
try {
    ...
    if ($existingProfile -notmatch "FORENSIC TOOLKIT") {
        ...
        if ($existingProfile) {
            ...
            Write-Host "[OK] Existing profile backed up to: $backupPath" -ForegroundColor Green
        }
...
"@
        ...
        Write-Host "[OK] PowerShell profile updated successfully" -ForegroundColor Green
    } else {
        Write-Host "[OK] Toolkit already configured in profile" -ForegroundColor Green
    }
} catch {
...
try {
    ...
    if ($functions.Count -gt 0) {
        Write-Host "[OK] Forensic functions available ($($functions.Count) functions)" -ForegroundColor Green
        $testsPassed++
    } else {
        throw "No forensic functions found"
    }
} catch {
    Write-Host "[FAIL] Forensic functions test failed: $($_.Exception.Message)" -ForegroundColor Red
}
...
if (Test-Path $toolsDir) {
    ...
    if ($toolFiles) {
        Write-Host "[OK] Tools directory created with $($toolFiles.Count) files" -ForegroundColor Green
        $testsPassed++
    } else {
        Write-Host "[OK] Tools directory created (empty)" -ForegroundColor Green
        $testsPassed++
    }
} else {
    Write-Host "[FAIL] Tools directory not found" -ForegroundColor Red
}
...
try {
    ...
    Write-Host "[FAIL] Python test failed: $($_.Exception.Message)" -ForegroundColor Red
}
